...
export async function createWpCloudReleaseManifest({ distPath, deployment, releaseId }) {
...
export async function buildWpCloudSftpBatch({ distPath, deployment, releaseId, bridgeMode = "install" }) {
  const source = path.resolve(distPath);
  ...
  await createWpCloudReleaseManifest({ distPath: source, deployment, releaseId });
...
async function uploadWpCloudRelease(distPath, deployment, id, bridgeMode) {
  ...
  const source = path.resolve(distPath);
  const prepared = await buildWpCloudSftpBatch({ distPath: source, deployment, releaseId: id, bridgeMode });
  let transfer;
  try {
...
async function activateWpCloudRelease({ deployment, id, site, password }) {
  const body = await wpCloudBridgeRequest({
    site,
    password,
    route: "release/activate",
    body: { release_id: id, public_url: deployment.public_url },
  });
...
async function finalizeWpCloudBridge({ deployment, site, password, plan, uploaded }) {
  ...
  return { mode: plan.mode, version: verified.bridge };
}
...
export async function rollbackWpCloudRelease({ deployment, site, password }) {
  ...
  const result = await wpCloudBridgeRequest({ site, password, route: "release/rollback" });
...
export async function deployFrontend({ distPath, deployment, releaseId = null, site = null, password = null }) {
  ...
  await createWpCloudReleaseManifest({ distPath, deployment, releaseId: id });
  const bridgePlan = await prepareWpCloudBridge({ site, password });
  const uploaded = await uploadWpCloudRelease(distPath, deployment, id, bridgePlan.mode);
  const bridge = await finalizeWpCloudBridge({ deployment, site, password, plan: bridgePlan, uploaded });
  const activation = await activateWpCloudRelease({ deployment, id, site, password });
  return { ...uploaded, bridge, activation, active: true };
}

export async function verifyPublicDigest({ url, digest = null, releaseId = null, attempts = 12 }) {
  if (!digest && !releaseId) throw new DashlessError("verification_target_missing", "Public verification requires a content digest or release ID.");
  let last = null;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      last = { status: response.status, found_digest: match?.[1] || null, found_release_id: foundReleaseId };
      if (response.ok && digestMatches && releaseMatches) return { verified: true, url, status: response.status, digest, release_id: releaseId };
    } catch (error) {
      last = { error: error.message };
    }
    if (attempt + 1 < attempts) await new Promise((resolve) => setTimeout(resolve, 500 * (attempt + 1)));
  }
  return { verified: false, url, expected_digest: digest, expected_release_id: releaseId, last };
}
