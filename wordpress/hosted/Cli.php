...
final class Cli {
    public static function register(App $app): void {
        ...
        \WP_CLI::add_command('dashless cancel-job',function($args)use($app){self::command(fn()=>$app->jobs->cancelQueued(Support::uuid($args[0]??'')));});
        ...
        \WP_CLI::add_command('dashless prune-previews',function($args,$assoc)use($app){self::command(fn()=>['pruned'=>$app->prunePreviews((int)($assoc['limit']??100))]);});
    }
