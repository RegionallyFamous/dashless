...
The canonical Hub publication is `npm run publish:hub`, called by the guarded `Publish Hub Astro frontend` workflow. It builds the Hub and six demos, uploads one immutable release to the WP Cloud release directory, switches `current.json` last, verifies the public Hub, demos, policy routes, and preview assets, and restores the previous pointer if verification fails. Cleanup removes only manifest-listed superseded releases and retains the newest verified previous release for rollback. Set `DASHLESS_RELEASE_ENABLED=true` only after the WP Cloud SSH secrets are present; otherwise the workflow stays intentionally skipped. Customer site builds remain a separate queued runtime path and never publish the canonical Hub release.
