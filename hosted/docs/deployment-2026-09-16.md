...
Follow-up verification on 2026-09-18 corrected the task command and completed a real customer-site plugin activation: WP Cloud task `760940` installed and activated the pinned site package on `smallproblems.dashless.blog` (site `152161766`). The authenticated hosted route then returned the expected contract envelope and a 401 for an unauthenticated capabilities probe, proving the plugin is active without exposing capabilities. Evidence: `../evidence/live-rollout-2026-09-18.json`.
