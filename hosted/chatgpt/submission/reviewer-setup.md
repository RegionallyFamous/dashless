...
2. Create two WordPress **subscriber** users with strong unique random passwords, purpose-limited email aliases and `dashless_email_verified=true` set administratively after identity ownership is established. Use WordPress's ordinary `/wp-login.php` password authentication for these preverified test users so reviewers are not blocked by email links, SMS or MFA. The OAuth consent endpoint uses the same real cookie authentication and nonce flow; do not add a bypass or reusable magic token.
