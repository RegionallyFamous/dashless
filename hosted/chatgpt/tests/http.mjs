...
check(['none','client_secret_basic','client_secret_post'].every(method=>data.token_endpoint_auth_methods_supported.includes(method)),'OAuth discovery advertises public and confidential token authentication');
check(data.issuer!==base && data.authorization_endpoint===data.issuer+'authorize','Auth0 is the authority, not a second local token issuer');
const authority=data.issuer;
r=await fetch(base+'/auth/login?return='+encodeURIComponent('/preview/?preview=test&handoff=private'),{redirect:'manual'});
const login=new URL(r.headers.get('location'));check(r.status===303 && login.origin+'/'===authority && login.searchParams.get('code_challenge_method')==='S256' && login.searchParams.has('nonce'),'browser sign-in redirects to Auth0 with PKCE and nonce');
check(r.headers.get('set-cookie').includes('HttpOnly') && r.headers.get('set-cookie').includes('SameSite=Lax') && !r.headers.get('set-cookie').includes('Domain='),'sign-in transaction cookie is host-only and HttpOnly');
check(r.headers.get('cache-control').includes('no-store'),'sign-in response is never cached');
r=await fetch(base+'/auth/callback?code=forged&state=forged');check(r.status===403,'forged login callback fails closed');
r=await fetch(base+'/auth/logout',{redirect:'manual'});check(r.status===403,'sign-out requires account nonce');
for(const path of ['/oauth/authorize','/oauth/token','/oauth/revoke','/auth/wordpress/start','/auth/wordpress/callback']){r=await fetch(base+path,{redirect:'manual'});check(r.status===410,'retired login endpoint '+path);}
...
r=await call({jsonrpc:'2.0',id:2,method:'tools/list'});data=await r.json();check(data.result.tools.length===28,'public tool scan works before account linking');
...
r=await call('{}',{},'GET');check(r.status===401 && r.headers.get('www-authenticate')?.includes('oauth-protected-resource'),'anonymous GET discovers OAuth before stream rejection');
r=await call('{}',{},'DELETE');check(r.status===405 && r.headers.get('allow')==='POST','unsupported method remains rejected');
...
r=await call('{}',{'Content-Type':'text/plain'});check(r.status===401 && r.headers.has('www-authenticate'),'unauthenticated non-JSON probe discovers OAuth');
r=await call('',{'Content-Type':'application/octet-stream',Accept:'*/*'});check(r.status===401 && r.headers.has('www-authenticate'),'portal empty POST discovers OAuth before transport validation');
