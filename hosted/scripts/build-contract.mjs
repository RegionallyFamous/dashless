...
const add=(name,description,scope,properties={},required=[],read=false)=>tools.push({name,description,scope,inputSchema:{type:'object',properties,additionalProperties:false,...(required.length?{required}: {})},annotations:{readOnlyHint:read,destructiveHint:scope==='blog:publish',openWorldHint:scope==='blog:publish'}});
...
add('list_themes','Discover Dashless themes during setup or redesign. Offer these named starting points and preview images; customers can customize them.','blog:read',{},[],true);
add('get_theme','Read one theme, its version, design defaults, preview image and customization options.','blog:read',{theme_id:str},['theme_id'],true);
...
const changes={type:'object',additionalProperties:false,properties:{
 theme_id:{type:'string',minLength:1,maxLength:80},theme_version:{type:'integer',minimum:1},
...
add('update_design','Stage design changes; does not publish. Set theme_id and theme_version from get_theme to apply its defaults; explicit palette, typography and layout in the same changes override those defaults. Identity and content are preserved. Omit theme fields for later customization.','blog:write',{expected_version:{type:'integer',minimum:0},client_key:key,changes},['expected_version','client_key','changes']);
