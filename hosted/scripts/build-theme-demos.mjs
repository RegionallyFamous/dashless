...
for (const [theme, slug] of themes) {
  ...
  process.env.DASHLESS_DEMO_THEME = theme;
  await run(theme, slug);
  await cp(canonicalDist, path.join(outputRoot, slug), { recursive: true });
}
